                                            ...
                                            <div class="parsley-row">
                                                <select onchange="location = this.value;" id="d_form_select_country" data-md-selectize required>
                                                    ...
                                                    @foreach($branch as $value)
                                                        @if($value->id==$id)
                                                            <option value="{{ route('fingerprint_index',$value->id) }}" selected>{!! $value->branch_name !!}</option>
                                                        @else
                                                            <option value="{{ route('fingerprint_index',$value->id) }}">{!! $value->branch_name !!}</option>
                                                        @endif
                                                    @endforeach
                                        ...
                                        <div class="uk-width-1-2">
                                            <div class="parsley-row">
                                                <select id="d_form_select_country" data-md-selectize required>
                                                    @foreach($branch as $value)
                                                        <option value="{{ route('fingerprint_index',$value->id) }}" selected disabled>{!! $value->branch_name !!}</option>
                                                    @endforeach
                                ...
                                <table class="uk-table" cellspacing="0" width="100%" id="data_table" >
                                   ...
                                   ?>
                                    ...
                                    @foreach($recruit as $value)
                                            <tr>
                                                <td>{!! $i++ !!}</td>
                                                <td>{!! $value->finger['date'] !!}</td>
                                                <td>{!! $value->paxid !!}</td>
                                                <td>{!! $value->finger['number'] !!}</td>
                                                ...
                                                @if($value->id==$value->finger['paxid'])
                                                    <td class="uk-text-center">
                                                        <a href="{!! route('fingerprint_edit',$value->id) !!}" class="batch-edit"><i class="md-icon material-icons uk-margin-right">&#xE254;</i></a>
                                                    </td>
                                                @else
                                                    <td class="uk-text-center">
                                                        <a href="{!! route('fingerprint_create',$value->id) !!}" class="batch-edit"><i class="md-icon material-icons uk-margin-right">+</i></a>
                                                    </td>
                                                @endif
                                            </tr>
