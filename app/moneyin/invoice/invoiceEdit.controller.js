...
function InvoiceEditController($q, $scope, $http) {
    ...
    $http
        ...
        .then(function(response){
            ...
            angular.forEach($scope.invoice_entries, function(invoice_entry) {
                ...
                $("#discount_type_" + dd++).val(invoice_entry.discount_type);
                // console.log(invoice_entry.discount_type);
                ...
                $scope.description[0] = invoice_entry.description;
            ...
            // $scope.vat_show=$scope.total_amount-$scope.sub_total-$scope.tax_total-$scope.adjustment-$scope.shipping_charge;
            $scope.vat_show=$scope.total_amount-$scope.sub_total-$scope.adjustment-$scope.shipping_charge;
            $scope.vat=(($scope.vat_show*100)/$scope.sub_total).toFixed(2);
            if ($scope.tax_total!=$scope.vat_show) {
                $scope.tax_total=$scope.tax_total+$scope.vat_show;
            }
            else{
    ...
    $scope.calculateInvoice = function () {
        ...
        $scope.vat_show =((parseFloat($scope.vat)*parseFloat($scope.sub_total))/100);
        $scope.tax_total = parseFloat(tax1);
        $scope.tax_total=$scope.vat_show;

        $scope.getAdjustment($scope.adjustment);

        $scope.total_amount = parseFloat(sub_total1)+ parseFloat($scope.vat_show) + parseFloat($scope.shipping_charge) + parseFloat($scope.adjustment) + parseFloat(tax1);
    }
